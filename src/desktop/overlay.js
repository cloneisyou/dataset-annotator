/**
 * Overlay rendering for keyboard and mouse visualization — Desktop
 */
...
// macOS label overrides: vkCode -> label
const MACOS_LABELS = {
  0x5b: "CMD", 0x5c: "CMD",   // WIN -> CMD
  0xa4: "OPT", 0xa5: "OPT",   // ALT -> OPT
};

let _osLabelOverrides = null;

/** Set OS to adjust keyboard labels (e.g. WIN->CMD, ALT->OPT for macOS) */
export function setKeyboardOS(os) {
  _osLabelOverrides = os === "macOS" ? MACOS_LABELS : null;
}
...
export function drawKeyboard(ctx, x, y, pressedKeys) {
  ...
  for (const [row, col, width, label, vk, isArrow] of KEYBOARD_LAYOUT) {
    ...
    const displayLabel = (_osLabelOverrides && _osLabelOverrides[vk]) || label;
    ...
    if (isArrow) {
      drawArrow(ctx, kx + kw / 2, ky + KEY_SIZE / 2, displayLabel);
    } else {
      ctx.font = `bold ${displayLabel.length <= 1 ? 14 : displayLabel.length <= 3 ? 10 : 8}px system-ui`;
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText(displayLabel, kx + kw / 2, ky + KEY_SIZE / 2);
    }
  }
}
